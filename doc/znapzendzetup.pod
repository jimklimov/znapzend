...
and where 'command' and its unique options is one of the following:
    ...
    create  [--recursive] [--synczbe] \
            [--mbuffer=<path>[:<port>]] [--mbuffersize=<size>] \
    ...
    edit    [--recursive=on|off] [--synczbe=on|off] \
...
An interesting niche option is B<synczbe>, useful for datasets (or trees
thereof) with operating environments -- main system, containers, maybe
virtual machine disks. As those environments are upgraded over time,
many OS workflows are prepared to B<zfs clone> an existing filesystem
tree, install packages or apply patches into the clone, and B<zfs promote>
it to become the new state of the system to reboot into. The B<synczbe>
option allows znapzend to align with such workflow by applying equivalent
operations to clone and promote datasets on the destination.
